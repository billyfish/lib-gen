...
BOOL WriteMakefile (CONST CONST_STRPTR  makefile_s, CONST CONST_STRPTR library_s, const struct List * const fn_defs_p)
{
	...
	if (makefile_p)
		{
			if (WriteMakefileHeader (makefile_p, library_s))
				{
					if (WriteMakefileSources (makefile_p, library_s))
						{
...
BOOL WriteMakefileHeader (BPTR makefile_p, CONST CONST_STRPTR library_s)
{
	...
	if (IDOS->FPrintf (makefile_p, "%s", MAKEFILE_CODE_HEADER (library_s) >= 0))
		{
			success_flag = TRUE;
		}
...
BOOL WriteMakefileSources (BPTR makefile_p, CONST CONST_STRPTR library_s, CONST CONST_STRPTR dir_s, const struct List * const src_files_p)
{
	BOOL success_flag = TRUE;
	struct Node *node_p = IExec->GetHead (fn_defs_p);
